function scp()
    vim.cmd[[
        "设置编译和自动保存文件
        let scopycmd=expand('~/develop/c.tools/sendfile.sh')
        let shouldidocopy = 1
        nnoremap <leader>sc :let shouldidocopy = 1<cr>
        nnoremap <leader>scn :let shouldidocopy = 0<cr>
        ...
        function! DoSCopy()
            ...
            let index_in_localfile = match(localfile, expand('~/develop/a.source/a.spdk/b.inflightio'))
            ...
            if !empty(localfile_other) && g:shouldidocopy == 1
                let index_in_localfile_other = match(localfile_other, expand('~/develop/a.source/a.spdk/b.inflightio'))
        ...
        augroup scopytoremote
            autocmd!
                ...
                autocmd BufWritePost *.py call DoSCopy()
        augroup END
